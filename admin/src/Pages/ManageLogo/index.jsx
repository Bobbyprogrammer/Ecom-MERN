...
const ManageLogo = () => {
  ...
  const removeImg = async (image, index) => {
    ...
    try {
      ...
      await deleteImages(`/api/logo/deteleImage?img=${image}`);

      // DELETE DATABASE RECORD
      await deleteData(`/api/logo/${logoId}`);
      ...
      context.alertBox("success", "Logo deleted successfully");
    } catch (error) {
      console.log(error);
  ...
  return (
    <>
      <div className="flex items-center justify-between px-2 py-0 mt-3">
        <h2 className="text-[18px] font-[600]">Manage Logo</h2>
      </div>
        ...
        <div className="card my-4 pt-5 pb-5 shadow-md sm:rounded-lg bg-white w-[100%] sm:w-[100%] lg:w-[65%] p-5">
          {previews?.length !== 0 &&
            previews?.map((image, index) => {
              return (
                <div
                  className="uploadBoxWrapper w-[150px] mr-3 relative"
                  key={index}
                >
                  <span
