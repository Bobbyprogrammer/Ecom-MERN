...
export const ProductDetailsComponent = (props) => {
  ...
  const addToCart = (product, userId, quantity) => {
    ...
    if (
      ...
      (props?.item?.productWeight?.length !== 0 && selectedWeight === "") ||
      (props?.item?.productRam?.length !== 0 && selectedRam === "")
    ) {
      setTabError(true);

      context?.alertBox("error", "Please select product variation");
  ...
  return (
    <>
      ...
      <div
        className="mt-3 pr-10 mb-5 text-[15px] leading-7 text-gray-700 product-description"
            ...
            {props?.item?.productRam?.map((item, index) => {
              return (
                <Button
                  key={index}
                  className={`${
                    selectedRamIndex === index ? "!bg-primary !text-white" : ""
                  } ${tabError === true && "error"}`}
                  onClick={() => {
                    setSelectedRam(item);
            ...
            {props?.item?.size?.map((item, index) => {
              return (
                <Button
                  key={index}
                  className={`${
                    selectedSizeIndex === index ? "!bg-primary !text-white" : ""
                  } ${tabError === true && "error"}`}
                  onClick={() => {
